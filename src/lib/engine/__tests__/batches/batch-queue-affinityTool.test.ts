...
const TYPE = "n8n-nodes-base.affinityTool";
...
async function runNode(
  ...
  const node = makeNode({ name: "Affinity Tool", type: TYPE, parameters: params });
  const ctx = createExecutionContext({
    node,
    workflow: {
      id: "wf-affinity-tool",
      name: "Affinity Tool Test",
      active: false,
      nodes: [node],
      connections: {},
      settings: {},
...
describe("n8n-nodes-base.affinityTool", () => {
  ...
  it("listEntry → create makes correct API call", async () => {
    const mockEntry = { id: 100, list_id: 5, entity_id: 42 };
    installFetch(mockResponse(mockEntry));
    const [out] = await runNode({ resource: "listEntry", operation: "create", listId: 5, entityId: 42 });
    expect(out).toHaveLength(1);
  ...
  it("listEntry → delete returns empty on 204", async () => {
    installFetch(mockResponse({}, { status: 204 }));
    const [out] = await runNode({ resource: "listEntry", operation: "delete", listId: 5, listEntryId: 99 });
    expect(out).toHaveLength(1);
  ...
  it("person → getAll returns persons", async () => {
    const mockPersons = [{ id: 1, first_name: "John", last_name: "Doe", emails: ["john@example.com"] }];
    installFetch(mockResponse(mockPersons));
    const [out] = await runNode({ resource: "person", operation: "getAll", returnAll: false, limit: 5 });
    expect(out).toHaveLength(1);
