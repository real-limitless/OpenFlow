...
function makeCtx(
  ...
  const node = makeNode({ name: "GotifyTest", type: TYPE, parameters });
  const defaultCreds = {
    gotifyApi: {
...
describe("batch-queue gotify — n8n-nodes-base.gotify", () => {
  ...
  it("creates a message on create operation", async () => {
    const out = await runGotify(
      { operation: "create", message: "Hello Gotify", priority: 5 },
      [{ json: {} }],
    );
    expect(out[0][0].json).toHaveProperty("message", "Hello Gotify");
    expect(out[0][0].json).toHaveProperty("priority", 5);
  });

  it("creates a message with only required field (default priority 0)", async () => {
    const out = await runGotify({ operation: "create", message: "Minimal" }, [
      { json: {} },
    ]);
    expect(out[0][0].json).toHaveProperty("message", "Minimal");
    expect(out[0][0].json).toHaveProperty("priority", 0);
  });

  it("creates a message with expression-resolved title", async () => {
    const out = await runGotify(
      { operation: "create", message: "={{ $json.alertText }}", title: "Server Alert", priority: 8 },
      [{ json: { alertText: "Disk space low" } }],
    );
    expect(out[0][0].json).toHaveProperty("title", "Server Alert");
    expect(out[0][0].json).toHaveProperty("priority", 8);
  });

  it("deletes a message on delete operation and merges original input", async () => {
    const out = await runGotify({ operation: "delete", messageId: "42" }, [
      { json: { msgId: 42 } },
    ]);
  ...
  it("fails when required message param is missing for create", async () => {
    await expect(runGotify({ operation: "create" }, [{ json: {} }])).rejects.toThrow(
      /message is required/,
    );
  });
  ...
  it("returns error items on continueOnFail for missing credential", async () => {
    ...
    const ctxNoCreds = createExecutionContext({
      node,
      workflow: {
        id: "test",
        name: "Test",
    ...
    const executor = getExecutor(TYPE)!;
    const out = await executor(ctxNoCreds, node);
