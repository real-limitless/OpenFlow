...
const TYPE = "n8n-nodes-base.perplexity";
...
describe("batch-queue perplexity — n8n-nodes-base.perplexity", () => {
  it("is registered as executor + description", () => {
    expect(hasExecutor(TYPE)).toBe(true);
    expect(getNodeType(TYPE).displayName).toBe("Perplexity");
  });

  it("resolves the same executor under the canonical type string", () => {
    const canonical = getExecutor(TYPE);
    expect(canonical).toBeDefined();
    expect(getExecutor("nodes-base.perplexity")).toBe(canonical);
  });
  ...
  it("chat — complete returns chat completion shape", async () => {
    ...
    expect(out[0][0].json.id).toBe("chat-123");
  ...
  it("chat — simplify collapses output", async () => {
    const fakeResponse = {
      id: "chat-456",
      object: "chat.completion",
      created: 1710000001,
    ...
    expect(out[0][0].json).toHaveProperty("created");
    expect(out[0][0].json).toHaveProperty("citations");
    expect(out[0][0].json).toHaveProperty("message");
    expect(out[0][0].json.message).toContain("Hello");
  });
  ...
  it("search — search returns results", async () => {
    ...
    expect(out[0]).toHaveLength(1);
    expect(out[0][0].json.id).toBe("search-789");
  ...
  it("agent — createResponse returns agent output", async () => {
    ...
    expect(out[0]).toHaveLength(1);
    expect(out[0][0].json.id).toBe("agent-001");
  ...
  it("multi-item pass-through produces one output per input", async () => {
    ...
    expect(out[0][0].json.id).toBe("chat-multi");
    expect(out[0][1].json.id).toBe("chat-multi");
    expect(calls).toHaveLength(2);
  });

  it("continueOnFail with unknown resource yields error item", async () => {
    installFetch({});
