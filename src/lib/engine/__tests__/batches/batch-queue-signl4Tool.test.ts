...
const TYPE = "n8n-nodes-base.signl4Tool";
...
describe("batch-queue signl4Tool — n8n-nodes-base.signl4Tool", () => {
  it("is registered as executor + description", () => {
    expect(hasExecutor(TYPE)).toBe(true);
    expect(getExecutor(TYPE)).toBeDefined();
    expect(getNodeType(TYPE).displayName).toBe("SIGNL4 (AI Tool)");
  });

  describe("send", () => {
    it("sends a basic alert using Tool-style parameters", async () => {
    ...
    it("sends an alert with additional fields", async () => {
      ...
      const out = await run(
        ...
        [{ message: "Security breach detected", service: "Infrastructure", externalId: "SEC-2024-001" }],
      );
      expect(calls).toHaveLength(1);
      expect(calls[0].headers["X-S4-Service"]).toBe("Infrastructure");
      expect(calls[0].headers["X-S4-AlertingScenario"]).toBe("multi_ack");
      expect(calls[0].headers["X-S4-ExternalID"]).toBe("SEC-2024-001");
      expect(calls[0].headers["X-S4-Filtering"]).toBe("true");
  ...
  describe("resolve", () => {
    it("resolves an alert by external ID", async () => {
      ...
      expect(calls[0].headers["X-S4-ExternalID"]).toBe("INCIDENT-2024-001");
      expect(out[0][0].json).toHaveProperty("eventId");
    });
  });
  ...
  describe("errors", () => {
    it("throws 404 on invalid team secret", async () => {
      ...
      await expect(
        run(
          { resource: "alert", operation: "send", message: "test" },
          [{ message: "test" }],
        ),
      ).rejects.toThrow("SIGNL4 Tool: invalid credentials");
    });
    ...
    it("throws on HTTP 400", async () => {
      ...
      ).rejects.toThrow("SIGNL4 Tool: request body was empty or missing");
    });
    ...
    it("throws on missing credential", async () => {
      ...
      ).rejects.toThrow("SIGNL4 Tool: signl4Api credential is required");
    });
    ...
    it("throws when externalId is missing for resolve", async () => {
      await expect(
        run(
          { resource: "alert", operation: "resolve" },
          [{}],
        ),
      ).rejects.toThrow("SIGNL4 Tool: externalId is required for resolve operation");
    });
  });
});
