...
const TYPE = "n8n-nodes-base.postBin";
...
describe("batch-queue postBin — n8n-nodes-base.postBin", () => {
  it("is registered as executor + description", () => {
    expect(hasExecutor(TYPE)).toBe(true);
    expect(getNodeType(TYPE).displayName).toBe("PostBin");
  });

  it("resolves the same executor under the canonical type string", () => {
    const canonical = getExecutor(TYPE);
    expect(canonical).toBeDefined();
    expect(getExecutor("nodes-base.postBin")).toBe(canonical);
  });
  ...
  it("invalid binId format throws", async () => {
    installFetch({});
    await expect(
      runNode(TYPE, { resource: "bin", operation: "get", binId: "invalid!" }, [{}]),
  ...
  it("API error throws when continueOnFail is false", async () => {
    installFetch({});
    await expect(
      runNode(TYPE, { resource: "bin", operation: "create", binId: "" }, [{}]),
  ...
  it("continueOnFail with API error yields error item", async () => {
    ...
    const { out } = await runNodeWithCtx(
      TYPE,
      { resource: "bin", operation: "create", continueOnFail: true },
      [{}],
      { continueOnFail: true },
    );
    expect(out[0]).toHaveLength(1);
