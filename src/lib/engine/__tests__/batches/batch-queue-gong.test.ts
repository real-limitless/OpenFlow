...
const TYPE = "n8n-nodes-base.gong";
...
describe("batch-queue gong — n8n-nodes-base.gong", () => {
  it("is registered as executor + description", () => {
    expect(hasExecutor(TYPE)).toBe(true);
    expect(getExecutor(TYPE)).toBeDefined();
    expect(getNodeType(TYPE).displayName).toBe("Gong");
  });

  it("resolves the executor under the canonical type string", () => {
    const canonical = getExecutor(TYPE);
    expect(canonical).toBeDefined();
    expect(getExecutor("nodes-base.gong")).toBe(canonical);
  });
  ...
  describe("call — getMany", () => {
    it("returns calls array with date filter", async () => {
      ...
      expect(calls[0].url).toContain("fromDateTime=2025-01-01T00%3A00%3A00Z");
      expect(calls[0].url).toContain("toDateTime=2025-01-31T23%3A59%3A59Z");
