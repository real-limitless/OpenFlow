...
const TYPE = "n8n-nodes-base.onfleet";
...
async function runNodeTest(
  ...
  const node = makeNode({ name: "Onfleet", type: TYPE, parameters });
...
describe("Onfleet node", () => {
  ...
  it("description is registered", () => {
    ...
    expect(desc?.displayName).toBe("Onfleet");
    expect(desc?.credentials).toContainEqual({ name: "onfleetApi" });
  });

  it("Create a task and verify output shape", async () => {
    ...
    expect(out[0][0].json.recipients).toBeInstanceOf(Array);
  });
  ...
  it("Get organization details", async () => {
    ...
    expect(out[0][0].json.id).toBe("org-1");
    expect(out[0][0].json.name).toBe("Test Org");
    expect(out[0][0].json.email).toBe("admin@test.com");
    expect(out[0][0].json.timezone).toBe("America/New_York");
  });

  it("Create a destination with parsed address", async () => {
    installFetch([mockResponse({
      id: "dest-1",
      address: { number: "2829", street: "Vallejo St" },
      location: [-122.4376, 37.7983],
    })]);
    const out = await runNodeTest({
      resource: "Destination",
      operation: "Create",
      address: { number: "2829", street: "Vallejo St", city: "San Francisco", state: "CA", country: "USA" },
    });
    expect(out[0][0].json.id).toBe("dest-1");
    expect(out[0][0].json.address).toBeDefined();
    expect(out[0][0].json.location).toBeInstanceOf(Array);
    expect(out[0][0].json.location).toHaveLength(2);
  });
