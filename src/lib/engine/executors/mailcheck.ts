...
export const mailcheckExecutor: NodeExecutor = async (ctx, node) => {
  ...
  if (!apiKey) {
    throw new Error("Mailcheck: mailcheckApi credential is not configured");
  }
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    try {
      ...
      if (!email) throw new Error("Mailcheck: email parameter is required");
...
async function mailcheckRequest(
  ...
  try {
    ...
    if (response.status === 429) {
      throw new Error("Mailcheck: rate limited (HTTP 429)");
    }
    if (response.status < 200 || response.status >= 300) {
      const desc = obj.message ? String(obj.message) : `HTTP ${response.status}`;
      throw new Error(`Mailcheck: ${desc}`);
    }
    if (obj.errors) {
