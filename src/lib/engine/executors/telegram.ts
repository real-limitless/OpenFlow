...
export const telegramExecutor: NodeExecutor = async (ctx, node) => {
...
async function runOperation(
  ...
  if (resource === "message") {
    return runMessageOperation(ctx, token, node, operation, itemJson, item);
  }
...
async function runCallbackOperation(
  ...
  if (operation === "answerQuery") {
    ...
    return asObj(await tgRequest(token, "answerCallbackQuery", body));
  }
...
async function runMessageOperation(
  ...
  if (operation === "sendAndWait") {
    return sendAndWait(token, node, itemJson, item);
  }
...
async function sendMessage(
  ...
  if (!text) throw new Error("Telegram: text is required");
  const additional = (node.parameters.additionalFields ?? {}) as Record<string, unknown>;
  const appendAttribution = additional.appendAttribution !== false;
  const finalText = appendAttribution ? `${text}${ATTRIBUTION_SUFFIX}` : text;
  const body: Record<string, unknown> = {
    chat_id: chatId,
    text: finalText,
    ...buildAdditionalFields(additional, itemJson),
...
async function sendMedia(
  ...
  const additional = (node.parameters.additionalFields ?? {}) as Record<string, unknown>;
  const binaryFile = Boolean(node.parameters.binaryFile ?? false);
  ...
  if (binaryFile) {
    ...
    if (additional.disableNotification !== undefined) {
      form.append(
        ...
        String(Boolean(resolveValue(additional.disableNotification, itemJson))),
      );
    }
...
async function sendAndWait(
  ...
  const typeOfApproval = String(node.parameters.typeOfApproval ?? "single");
  const buttonLabel = String(node.parameters.buttonLabel ?? "Approve");
  const declineButtonLabel = String(node.parameters.declineButtonLabel ?? "Decline");

  const keyboard: Record<string, unknown>[][] = [[{ text: buttonLabel, callback_data: "approve" }]];
  if (responseType === "approval" && typeOfApproval === "double") {
    keyboard[0].push({ text: declineButtonLabel, callback_data: "decline" });
  }
  const body: Record<string, unknown> = {
    chat_id: chatId,
    text: message,
  ...
  // TODO: implement putExecutionToWait + paired Telegram Trigger webhook resume.
  // On resume, output should contain { data: { approved, user } } from the
  // human response. For now, pass through the input item json.
