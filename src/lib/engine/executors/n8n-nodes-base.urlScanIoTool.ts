...
}, node: { parameters: Record<string, unknown> }): Promise<string> {
...
async function runOperation(
  ...
  if (operation === "Perform") return performScan(ctx, node, itemJson);
  if (operation === "Get") return getScan(ctx, node, itemJson);
  if (operation === "Get All") return getAllScans(ctx, node, itemJson);
  throw new Error(`urlscan.io: unsupported operation "${operation}"`);
}

export const urlScanIoToolExecutor: NodeExecutor = async (ctx, node) => {
