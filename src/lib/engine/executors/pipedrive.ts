...
export const pipedriveExecutor: NodeExecutor = async (ctx, node) => {
...
async function apiRequestFormData(
  ...
  try {
    ...
    const init: RequestInit = { method, headers, body: formData, signal: controller.signal };
    const response = await fetch(url, init);
    const text = await response.text();
    let parsed: unknown = text;
    try { parsed = text ? JSON.parse(text) : null; } catch { /* keep text */ }
...
async function runFileOperation(
  ...
  if (operation === "create") {
    ...
    if (binaryPropertyName && item.binary?.[binaryPropertyName]) {
      ...
      const res = await apiRequestFormData("POST", "/files", auth, formData);
      return { json: unwrapData(res) };
    }
