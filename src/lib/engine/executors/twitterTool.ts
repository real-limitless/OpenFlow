...
export const twitterToolExecutor: NodeExecutor = async (ctx, node) => {
...
async function getToken(ctx: ExecutionContext, node: INode): Promise<string> {
  ...
  if (!accessToken) {
    throw new Error(`X (Twitter) Tool: ${credName} credential is not configured`);
  }
  return accessToken;
}
...
async function runOperation(
  ...
  throw new Error(`X (Twitter) Tool: unsupported resource "${resource}"`);
}
...
async function runTweetOperation(
  ...
  if (operation === "create") {
    ...
    if (!text) throw new Error("X (Twitter) Tool: text is required for creating a tweet");
  ...
  if (operation === "reply") {
    ...
    if (!text || !tweetId) {
      throw new Error("X (Twitter) Tool: text and tweetId are required for a reply");
    }
  ...
  if (operation === "delete") {
    ...
    if (!tweetId) throw new Error("X (Twitter) Tool: tweetId is required");
  ...
  if (operation === "search") {
    ...
    if (!query) throw new Error("X (Twitter) Tool: searchQuery is required");
  ...
  if (operation === "like") {
    ...
    if (!userId) throw new Error("X (Twitter) Tool: userId is required from credential for like");
  ...
  if (operation === "retweet") {
    ...
    const userId = credential ? String(credential.userId ?? "") : "";
    if (!userId) throw new Error("X (Twitter) Tool: userId is required from credential for retweet");
  ...
  throw new Error(`X (Twitter) Tool: unsupported tweet operation "${operation}"`);
}
...
async function runDirectMessageOperation(
  ...
  if (operation === "create") {
    ...
    if (!recipientId || !text) {
      throw new Error("X (Twitter) Tool: recipientIdentifier and messageText are required for DM");
    }
    if (recipientId.startsWith("@")) {
      ...
      if (!recipientId) {
        throw new Error(`X (Twitter) Tool: could not resolve username "${username}"`);
      }
    }
    const res = await twitterRequest(token, "POST", `/dm_conversations/with/${recipientId}/messages`, { text });
    return { json: simplify ? asObj(res.data) : res };
  }

  throw new Error(`X (Twitter) Tool: unsupported DM operation "${operation}"`);
}
...
async function runUserOperation(
  ...
  if (operation === "get") {
    ...
    if (!identifier) throw new Error("X (Twitter) Tool: userIdentifier is required");
  ...
  throw new Error(`X (Twitter) Tool: unsupported user operation "${operation}"`);
}
...
async function runListOperation(
  ...
  if (operation === "addMember") {
    ...
    if (!listId || !memberId) {
      throw new Error("X (Twitter) Tool: listId and memberIdentifier are required");
    }
  ...
  throw new Error(`X (Twitter) Tool: unsupported list operation "${operation}"`);
}
...
async function twitterRequest(
  ...
  try {
    ...
    if (err instanceof Error && err.message.includes("X (Twitter) Tool")) {
      throw err;
    }
    if (err instanceof Error && !err.message.includes("X (Twitter) Tool")) {
      throw new Error(`X (Twitter) Tool request failed: ${err.message}`);
    }
    throw err;
  } finally {
    clearTimeout(timer);
