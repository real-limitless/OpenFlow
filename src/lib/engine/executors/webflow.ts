...
export const webflowExecutor: NodeExecutor = async (ctx, node) => {
  const items = ensureItems(ctx.getInputItems(0));
  const out: INodeExecutionData[] = [];

  const authentication = (node.parameters.authentication as string) ?? "accessToken";
  const resource = (node.parameters.resource as string) ?? "item";
  const operation = (node.parameters.operation as string) ?? "get";

  if (resource !== "item") {
    throw new Error(`Webflow: unsupported resource "${resource}"`);
  }
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    try {
      const result = await runOperation(ctx, node, operation, itemJson, authentication);
...
async function authHeaders(
  ...
  const credName = authentication === "oAuth2" ? "webflowOAuth2Api" : "webflowApi";
  const cred = await ctx.getCredential(credName);
  const accessToken = cred ? String(cred.accessToken ?? "") : "";
  if (!accessToken) {
    throw new Error(`Webflow: ${credName} credential is not configured`);
  }
  return {
...
async function webflowRequest(
  ...
  try {
    ...
    throw new Error(`Webflow request failed: ${err instanceof Error ? err.message : String(err)}`);
  } finally {
    clearTimeout(timer);
  }
}
