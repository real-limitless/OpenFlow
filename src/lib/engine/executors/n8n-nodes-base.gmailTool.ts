...
function buildEmailMime(
  ...
  if (isHtml) headers.push('Content-Type: text/html; charset="UTF-8"');
...
function buildMultipartEmail(
  ...
  const isHtml = opts.emailType === "html";
  if (isHtml) parts.push('Content-Type: text/html; charset="UTF-8"');
  else parts.push('Content-Type: text/plain; charset="UTF-8"');
...
async function getToken(ctx: ExecutionContext): Promise<string> {
  ...
  if (!accessToken) {
    throw new Error("Gmail Tool: gmailOAuth2 credential is not configured");
  }
  return accessToken;
}
...
async function gmailRequest(
  ...
  try {
    ...
    if (err instanceof Error && !err.message.includes("Gmail Tool:")) {
      throw new Error(`Gmail Tool request failed: ${err.message}`);
    }
    throw err;
  } finally {
    clearTimeout(timer);
...
export const gmailToolExecutor: NodeExecutor = async (ctx, node) => {
...
async function runOperation(
  ...
  throw new Error(`Gmail Tool: unsupported resource "${resource}"`);
}
...
async function runMessageOperation(
  ...
  if (operation === "send" || operation === "sendAndWait") {
    return sendMessage(token, node, itemJson, item);
  }
  if (operation === "get") {
  ...
  throw new Error(`Gmail Tool: unsupported message operation "${operation}"`);
}
...
async function sendMessage(
  ...
  if (!to) throw new Error("Gmail Tool: sendTo is required");
  ...
  const mime =
    attachments.length > 0
      ? buildMultipartEmail({ to, subject, body: message, emailType }, attachments)
      : buildEmailMime({ to, subject, body: message, emailType });
...
async function getMessage(
  ...
  if (!messageId) throw new Error("Gmail Tool: messageId is required");
...
async function runLabelOperation(
  ...
  if (operation === "create") {
    ...
    if (!name) throw new Error("Gmail Tool: name is required");
  ...
  throw new Error(`Gmail Tool: unsupported label operation "${operation}"`);
}
...
async function runDraftOperation(
  ...
  if (operation === "get") {
    ...
    if (!draftId) throw new Error("Gmail Tool: messageId (draft id) is required");
  ...
  throw new Error(`Gmail Tool: unsupported draft operation "${operation}"`);
}
...
async function runThreadOperation(
  ...
  if (operation === "get") {
    ...
    if (!threadId) throw new Error("Gmail Tool: threadId is required");
  ...
  throw new Error(`Gmail Tool: unsupported thread operation "${operation}"`);
}
