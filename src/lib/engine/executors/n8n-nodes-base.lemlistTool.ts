import type { NodeExecutor, ExecutionContext, INode, INodeExecutionData } from "@/sdk";
...
async function lemlistRequest(url: string, options: {
  ...
  try {
    ...
    if (response.status === 429) {
      throw new Error("Lemlist Tool: rate limited (HTTP 429)");
    }
    if (response.status < 200 || response.status >= 300) {
      const desc = obj.message ? String(obj.message) : `HTTP ${response.status}`;
      throw new Error(`Lemlist Tool: ${desc}`);
    }
    return obj;
  } finally {
    clearTimeout(timer);
...
async function getApiKey(ctx: ExecutionContext): Promise<string> {
  ...
  if (!apiKey) {
    throw new Error("Lemlist Tool: lemlistApi credential is not configured");
  }
  return apiKey;
}
...
export const lemlistToolExecutor: NodeExecutor = async (ctx, node) => {
...
async function runOperation(
  ...
  throw new Error(`Lemlist Tool: unsupported resource/operation "${resource}/${operation}"`);
}
...
async function campaignGetStats(
  ...
  if (!campaignId) throw new Error("Lemlist Tool: campaignId is required for getStats");
...
async function enrichmentGet(
  ...
  if (!enrichId) throw new Error("Lemlist Tool: enrichId is required for enrichment get");
...
async function enrichmentEnrichLead(
  ...
  if (!leadId) throw new Error("Lemlist Tool: leadId is required for enrichLead");
...
async function leadCreate(
  ...
  if (!campaignId || !email) throw new Error("Lemlist Tool: campaignId and email are required for lead create");
...
async function leadDelete(
  ...
  const campaignId = String(resolveValue(node.parameters.campaignId, itemJson) ?? "");
  const email = String(resolveValue(node.parameters.email, itemJson) ?? "");
  if (!campaignId || !email) throw new Error("Lemlist Tool: campaignId and email are required for lead delete");
...
async function leadGet(
  ...
  const apiKey = await getApiKey(ctx);
  const email = String(resolveValue(node.parameters.email, itemJson) ?? "");
  if (!email) throw new Error("Lemlist Tool: email is required for lead get");
...
async function leadUnsubscribe(
  ...
  const campaignId = String(resolveValue(node.parameters.campaignId, itemJson) ?? "");
  const email = String(resolveValue(node.parameters.email, itemJson) ?? "");
  if (!campaignId || !email) throw new Error("Lemlist Tool: campaignId and email are required for lead unsubscribe");
...
async function unsubscribeAdd(
  ...
  const apiKey = await getApiKey(ctx);
  const email = String(resolveValue(node.parameters.email, itemJson) ?? "");
  if (!email) throw new Error("Lemlist Tool: email is required for unsubscribe add");
...
async function unsubscribeDelete(
  ...
  const apiKey = await getApiKey(ctx);
  const email = String(resolveValue(node.parameters.email, itemJson) ?? "");
  if (!email) throw new Error("Lemlist Tool: email is required for unsubscribe delete");
