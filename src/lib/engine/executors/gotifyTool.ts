...
export const gotifyToolExecutor: NodeExecutor = async (ctx) => {
  ...
  if (operation === "create") {
    ...
    for (const item of items) {
      try {
        ...
        const title = additionalFields.title as string | undefined;
        if (title) body.title = title;
        const priority = (additionalFields.priority as number | undefined) ?? 0;
        body.priority = priority;
        const contentType = options.contentType as string | undefined;
  ...
  if (operation === "delete") {
    ...
    for (const item of items) {
      try {
        const messageId = ctx.getParam<number | undefined>("messageId");
  ...
  if (operation === "getAll") {
    try {
      ...
      if (!returnAll) {
        ...
        let nextUrl: string | undefined = `${baseUrl}?limit=100`;
