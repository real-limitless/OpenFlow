...
async function getAccessToken(ctx: ExecutionContext, node: INode): Promise<string> {
  ...
  if (!cred) {
    throw new Error(`GoogleCalendar: ${credName} credential is not configured`);
  }
  const accessToken = String(cred.accessToken ?? cred.access_token ?? "");
  if (!accessToken) {
    throw new Error(`GoogleCalendar: ${credName} has no accessToken`);
  }
  return accessToken;
}
...
async function apiRequest(
  ...
  if (res.status < 200 || res.status >= 300) {
    ...
    throw new Error(`GoogleCalendar: ${msg}`);
  }
...
async function calendarAvailability(
  ...
  if (!startTime || !endTime) {
    throw new Error("GoogleCalendar: startTime and endTime are required for availability");
  }
...
async function eventCreate(
  ...
  if (!startTime || !endTime) {
    throw new Error("GoogleCalendar: startTime and endTime are required for create");
  }
...
async function eventDelete(
  ...
  if (!eventId) throw new Error("GoogleCalendar: eventId is required for delete");
...
async function eventGet(
  ...
  if (!eventId) throw new Error("GoogleCalendar: eventId is required for get");
...
async function eventUpdate(
  ...
  if (!eventId) throw new Error("GoogleCalendar: eventId is required for update");
...
export const googleCalendarExecutor: NodeExecutor = async (ctx, node) => {
    ...
    try {
      ...
      if (resource === "calendar") {
        if (operation !== "availability") {
          throw new Error(`GoogleCalendar: unsupported calendar operation "${operation}"`);
        }
        ...
        if (operation === "create") {
          ...
          throw new Error(`GoogleCalendar: unsupported event operation "${operation}"`);
        }
      } else {
        throw new Error(`GoogleCalendar: unsupported resource "${resource}"`);
      }
    } catch (err) {
