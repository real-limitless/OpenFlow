...
export interface GroqAgentToolDef {
  name: string;
  description?: string;
  schema?: unknown;
}

export interface GroqModelHandle {
  type: "@n8n/n8n-nodes-langchain.lmChatGroq";
  model: string;
  options: Record<string, unknown>;
  invoke(
    messages: GroqChatMessage[],
    tools?: GroqAgentToolDef[] | unknown[],
  ): Promise<GroqCompletionResult>;
}

export type GroqHttpClient = (options: SdkHttpRequestOptions) => Promise<SdkHttpResponse>;

let httpOverride: GroqHttpClient | null = null;

export function setGroqHttpClient(factory: GroqHttpClient | null): void {
...
function resolveModelId(ctx: ExecutionContext): string {
  ...
  if (raw == null || raw === "") {
    throw new Error("Groq Chat Model: model id is required");
  }
  ...
  if (!modelId) {
    throw new Error("Groq Chat Model: model id resolved to empty");
  }
  return modelId;
}
...
function normalizeAgentToolDefs(tools: unknown[] | undefined): GroqAgentToolDef[] {
  if (!tools || tools.length === 0) return [];
  const out: GroqAgentToolDef[] = [];
...
function mapAgentToolsToGroq(tools: GroqAgentToolDef[]): unknown[] {
  return tools.map((t) => {
    const parameters =
...
function serializeMessagesForApi(messages: GroqChatMessage[]): unknown[] {
...
function buildRequestBody(
  ...
  if (temp != null) {
    const t = Number(temp);
    body.temperature = t === 0 ? 1e-8 : t;
  }
  if (options.maxTokens != null) body.max_tokens = options.maxTokens;
  if (agentTools && agentTools.length > 0) {
    body.tools = mapAgentToolsToGroq(agentTools);
  }
  return body;
}
...
function parseChatCompletionsResponse(body: unknown): GroqCompletionResult {
  const b = body as {
    choices?: Array<{
      message?: {
        content?: string | null;
...
function classifyError(status: number, body: unknown): Error {
  ...
  if (status === 429) {
    return new Error(
      "Groq rate limit exceeded — the service is receiving too many requests. Mitigate with batching or Wait nodes.",
    );
  }
  return new Error(`Groq API error (${status}): ${bodyStr}`);
}
...
async function invokeModel(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      throw new Error(`Groq request failed: ${lastError.message}`);
    }
...
export const lmChatGroqExecutor: NodeExecutor = async (ctx) => {
  const credentials = await requireCredential(ctx, "groqApi");
  const apiKey = String(credentials.apiKey ?? "");
  if (!apiKey) {
    throw new Error('Groq Chat Model: credential "groqApi" is missing apiKey');
  }

  const model = resolveModelId(ctx);
  const options = ctx.getParam<Record<string, unknown>>("options", {}) ?? {};

  const handle: GroqModelHandle = {
    type: "@n8n/n8n-nodes-langchain.lmChatGroq",
    model,
    options,
    invoke(
