...
async function getAccessToken(ctx: ExecutionContext): Promise<string> {
  ...
  if (!cred) {
    throw new Error("GoogleCloudStorageTool: googleCloudStorageOAuth2Api credential is not configured");
  }
  const accessToken = String(cred.accessToken ?? cred.access_token ?? "");
  if (!accessToken) {
    throw new Error("GoogleCloudStorageTool: credential has no accessToken");
  }
  return accessToken;
}
...
async function apiRequest(
  ...
  if (res.status < 200 || res.status >= 300) {
    ...
    throw new Error(`GoogleCloudStorageTool: ${msg}`);
  }
...
export const googleCloudStorageToolExecutor: NodeExecutor = async (ctx, node) => {
      ...
      if (resource === "bucket") {
        switch (operation) {
          ...
          case "update":
            ...
            break;
          default:
            throw new Error(`GoogleCloudStorageTool: unknown bucket operation "${operation}"`);
        }
        ...
        switch (operation) {
          ...
          case "update":
            ...
            break;
          default:
            throw new Error(`GoogleCloudStorageTool: unknown object operation "${operation}"`);
        }
      } else {
        throw new Error(`GoogleCloudStorageTool: unknown resource "${resource}"`);
      }
