...
async function getAuthHeaders(ctx: ExecutionContext): Promise<{ headers: Record<string, string>; instanceUrl: string }> {
  const cred = await ctx.getCredential("salesforceOAuth2Api") ??
    await ctx.getCredential("salesforceJwtApi");
  const token = cred
    ? String(cred.accessToken ?? cred.token ?? "")
    : "";
  if (!token) {
...
function buildFields(node: INode, itemJson: Record<string, unknown>): Record<string, unknown> {
  const fieldsRaw = node.parameters.fields as
    | { field?: Array<{ fieldName?: string; fieldValue?: string }> }
    | undefined;
  const fields: Record<string, unknown> = {};
  if (fieldsRaw?.field && Array.isArray(fieldsRaw.field)) {
    for (const f of fieldsRaw.field) {
      if (f.fieldName) {
        fields[f.fieldName] = resolveValue(f.fieldValue, itemJson);
      }
    }
  }
  return fields;
...
function buildFlowInputs(node: INode, itemJson: Record<string, unknown>): Record<string, unknown> {
  const inputsRaw = node.parameters.flowInputs as
    | { input?: Array<{ name?: string; value?: string }> }
    | undefined;
  const inputs: Record<string, unknown> = {};
  if (inputsRaw?.input && Array.isArray(inputsRaw.input)) {
    for (const inp of inputsRaw.input) {
      if (inp.name) {
        inputs[inp.name] = resolveValue(inp.value, itemJson);
      }
    }
  }
  return inputs;
...
export const salesforceExecutor: NodeExecutor = async (ctx, node) => {
...
async function sobjectCreate(
  ...
  return { id: result.id, success: result.success, ...fields };
}
...
async function flowInvoke(
  ...
  const inputs = buildFlowInputs(node, itemJson);
