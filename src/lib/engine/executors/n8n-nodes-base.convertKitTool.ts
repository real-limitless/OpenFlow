...
async function apiRequest(
  ...
  try {
    ...
    if (response.status < 200 || response.status >= 300) {
      ...
      const errMsg = (obj.error as string) ?? (obj.message as string) ?? `ConvertKit Tool API error: ${response.status}`;
...
export const convertKitToolExecutor: NodeExecutor = async (ctx, node) => {
...
async function runOperation(
  ...
  if (!apiKey) throw new Error("ConvertKit Tool: API Secret is required. Configure a convertKitApi credential.");
  ...
  switch (resource) {
    ...
    default: throw new Error(`ConvertKit Tool: unsupported resource "${resource}"`);
  }
}
...
async function runCustomFieldOperation(
  ...
  if (operation === "create") {
    ...
    if (!label) throw new Error("ConvertKit Tool: label is required for customField create");
  ...
  if (operation === "delete") {
    ...
    if (!fieldId) throw new Error("ConvertKit Tool: fieldId is required for customField delete");
  ...
  if (operation === "update") {
    const fieldId = String(resolveValue(node.parameters.fieldId, itemJson) ?? "");
    if (!fieldId) throw new Error("ConvertKit Tool: fieldId is required for customField update");
    const label = String(resolveValue(node.parameters.label, itemJson) ?? "");
    const res = await apiRequest("PATCH", `/custom_fields/${fieldId}`, apiKey, { label });
    return { json: { customField: res.custom_field ?? {} } };
  }

  throw new Error(`ConvertKit Tool: unsupported customField operation "${operation}"`);
}
...
async function runFormOperation(
  ...
  if (operation === "addSubscriber") {
    ...
    if (!formId || !email) throw new Error("ConvertKit Tool: formId and email are required for form addSubscriber");
  ...
  if (operation === "listSubscriptions") {
    const formId = String(resolveValue(node.parameters.formId, itemJson) ?? "");
    if (!formId) throw new Error("ConvertKit Tool: formId is required for form listSubscriptions");
    const res = await apiRequest("GET", `/forms/${formId}/subscriptions`, apiKey);
    return { json: { subscribers: res.subscribers ?? [] } };
  }

  throw new Error(`ConvertKit Tool: unsupported form operation "${operation}"`);
}
...
async function runSequenceOperation(
  ...
  if (operation === "addSubscriber") {
    ...
    const email = String(resolveValue(node.parameters.email, itemJson) ?? "");
    if (!sequenceId || !email) throw new Error("ConvertKit Tool: sequenceId and email are required for sequence addSubscriber");
    const tags = resolveValue(node.parameters.tags, itemJson);
    const body: Record<string, unknown> = { email };
    if (tags) body.tags = tags;
  ...
  if (operation === "listSubscriptions") {
    const sequenceId = String(resolveValue(node.parameters.sequenceId, itemJson) ?? "");
    if (!sequenceId) throw new Error("ConvertKit Tool: sequenceId is required for sequence listSubscriptions");
    const res = await apiRequest("GET", `/sequences/${sequenceId}/subscriptions`, apiKey);
    return { json: { subscribers: res.subscribers ?? [] } };
  }

  throw new Error(`ConvertKit Tool: unsupported sequence operation "${operation}"`);
}
...
async function runTagOperation(
  ...
  if (operation === "create") {
    ...
    if (!name) throw new Error("ConvertKit Tool: name is required for tag create");
  ...
  throw new Error(`ConvertKit Tool: unsupported tag operation "${operation}"`);
}
...
async function runTagSubscriberOperation(
  ...
  if (operation === "add") {
    ...
    if (!email || !tagId) throw new Error("ConvertKit Tool: email and tagId are required for tagSubscriber add");
  ...
  if (operation === "listSubscriptions") {
    const tagId = String(resolveValue(node.parameters.tagId, itemJson) ?? "");
    if (!tagId) throw new Error("ConvertKit Tool: tagId is required for tagSubscriber listSubscriptions");
  ...
  if (operation === "remove") {
    const email = String(resolveValue(node.parameters.email, itemJson) ?? "");
    const tagId = String(resolveValue(node.parameters.tagId, itemJson) ?? "");
    if (!email || !tagId) throw new Error("ConvertKit Tool: email and tagId are required for tagSubscriber remove");
    await apiRequest("DELETE", `/tags/${tagId}/unsubscribe`, apiKey, { email });
    return { json: { success: true } };
  }

  throw new Error(`ConvertKit Tool: unsupported tagSubscriber operation "${operation}"`);
}
