...
export const affinityExecutor: NodeExecutor = async (ctx, node) => {
  ...
  if (!cred) throw new Error("Affinity: no credential found for affinityApi");
...
async function runOperation(
  ...
  switch (resource) {
    ...
    default: throw new Error(`Affinity: unsupported resource "${resource}"`);
  }
}
...
async function runListOperation(
  ...
  if (operation === "getAll") {
    ...
    if (!returnAll && limit > 0) params.limit = String(limit);
    if (additionalFields?.orderBy) params.order_by = String(additionalFields.orderBy);
    if (additionalFields?.direction) params.direction = String(additionalFields.direction);
  ...
  throw new Error(`Affinity: unsupported list operation "${operation}"`);
}
...
async function runListEntryOperation(
  ...
  if (operation === "create") {
    ...
    const entityType = getNum(node.parameters.entityType, itemJson);
    const res = await apiRequest("POST", `/lists/${listId}/list-entries`, apiKey, {
      entity_id: entityId,
  ...
  if (operation === "delete") {
    const entryId = getNum(node.parameters.entryId, itemJson);
    const res = await apiRequest("DELETE", `/lists/${listId}/list-entries/${entryId}`, apiKey);
    return [{ json: Object.keys(res).length > 0 ? res : {} }];
  }

  if (operation === "get") {
    const entryId = getNum(node.parameters.entryId, itemJson);
    const res = await apiRequest("GET", `/lists/${listId}/list-entries/${entryId}`, apiKey);
    return [{ json: res }];
  }
  ...
  throw new Error(`Affinity: unsupported listEntry operation "${operation}"`);
}
...
async function runOrganizationOperation(
  ...
  if (operation === "create") {
    ...
    if (!name) throw new Error("Affinity: name is required for organization create");
    const body: Record<string, unknown> = { name };
    const additionalFields = node.parameters.additionalFields as Record<string, unknown> | undefined;
    if (additionalFields) {
      if (additionalFields.domain) body.domain = String(additionalFields.domain);
      if (additionalFields.domains) body.domains = String(additionalFields.domains).split(",").map(s => s.trim()).filter(Boolean);
      if (additionalFields.personIds) body.person_ids = parseArray(additionalFields.personIds);
    }
  ...
  if (operation === "get") {
    const organizationId = getNum(node.parameters.organizationId, itemJson);
    const res = await apiRequest("GET", `/organizations/${organizationId}`, apiKey);
    return [{ json: res }];
  }
  ...
  if (operation === "update") {
    ...
    if (Object.keys(body).length === 0) throw new Error("Affinity: at least one field required for organization update");
    const res = await apiRequest("PUT", `/organizations/${organizationId}`, apiKey, body);
    return [{ json: res }];
  }

  throw new Error(`Affinity: unsupported organization operation "${operation}"`);
}
...
async function runPersonOperation(
  ...
  if (operation === "create") {
    ...
    if (!firstName || !lastName) throw new Error("Affinity: firstName and lastName are required for person create");
    ...
    if (additionalFields) {
      ...
      if (additionalFields.organizationIds) body.organization_ids = parseArray(additionalFields.organizationIds);
    }
  ...
  if (operation === "get") {
    const personId = getNum(node.parameters.personId, itemJson);
    const res = await apiRequest("GET", `/persons/${personId}`, apiKey);
    return [{ json: res }];
  }
  ...
  if (operation === "update") {
    ...
    if (Object.keys(body).length === 0) throw new Error("Affinity: at least one field required for person update");
    const res = await apiRequest("PUT", `/persons/${personId}`, apiKey, body);
    return [{ json: res }];
  }

  throw new Error(`Affinity: unsupported person operation "${operation}"`);
}
