...
async function getWebDavUrl(ctx: ExecutionContext): Promise<string> {
  ...
  if (!cred) {
    throw new Error("Nextcloud: credential is not configured");
  }
  const url = String(cred.webDavUrl ?? cred.url ?? cred.baseUrl ?? "");
  if (!url) {
    throw new Error("Nextcloud: webDavUrl is not set in credential");
  }
  return url.replace(/\/+$/, "");
}
...
async function ocsRequest(
  ...
  try {
    ...
    throw new Error(`Nextcloud OCS error: ${res.status} ${res.statusText}`);
  }
...
async function runOperation(
  ...
  if (resource === "file") {
    if (operation === "upload") {
      return runFileUpload(node, headers, baseUrl, itemJson, binaryData);
    }
    if (operation === "download") {
      return runFileDownload(node, headers, baseUrl, itemJson);
    }
    if (operation === "copy") {
      return runFileCopy(node, headers, baseUrl, itemJson);
    }
    if (operation === "move") {
      return runFileMove(node, headers, baseUrl, itemJson);
    }
    if (operation === "delete") {
      return runFileDelete(node, headers, baseUrl, itemJson);
    }
    if (operation === "share") {
      return runFileShare(node, headers, baseUrl, itemJson);
    }
  }

  if (resource === "folder") {
    if (operation === "create") {
      return runFolderCreate(node, headers, baseUrl, itemJson);
    }
    if (operation === "list") {
      return runFolderList(node, headers, baseUrl, itemJson);
    }
    if (operation === "copy") {
      return runFileCopy(node, headers, baseUrl, itemJson);
    }
  ...
  if (resource === "user") {
    if (operation === "invite") {
      return runUserInvite(node, headers, baseUrl, itemJson);
    }
    if (operation === "delete") {
      return runUserDelete(node, headers, baseUrl, itemJson);
    }
    if (operation === "get") {
      return runUserGet(node, headers, baseUrl, itemJson);
    }
    if (operation === "getAll") {
      return runUserGetAll(node, headers, baseUrl, itemJson);
    }
    if (operation === "edit") {
      return runUserEdit(node, headers, baseUrl, itemJson);
    }
  }
...
async function runFileUpload(
  ...
  if (binaryData && binaryData[binaryProperty]) {
    const bd = binaryData[binaryProperty];
    content = bd.data ?? "";
  } else {
...
async function runFileDownload(
  ...
  return {
    data: body,
    contentLength: respHeaders["content-length"] ?? body.length,
...
async function runFileCopy(
  ...
  const sourcePath = resolveParam(node, "sourcePath");
  const destinationPath = resolveParam(node, "destinationPath");
  if (!sourcePath || !destinationPath) {
    throw new Error("Nextcloud: sourcePath and destinationPath are required for copy");
  }
  await webdavRequest(baseUrl, {
    ...headers,
    Destination: `${baseUrl}${WEBDAV_BASE}${destinationPath}`,
    "Overwrite": "F",
  }, "COPY", sourcePath);
...
async function runFileMove(
  ...
  const destinationPath = resolveParam(node, "destinationPath");
  if (!sourcePath || !destinationPath) {
    throw new Error("Nextcloud: sourcePath and destinationPath are required for move");
  }
  await webdavRequest(baseUrl, {
    ...headers,
    Destination: `${baseUrl}${WEBDAV_BASE}${destinationPath}`,
...
async function runFileShare(
  ...
  const result = await ocsRequest(baseUrl, headers, "/apps/files_sharing/api/v1/shares", "POST", params);
  return result;
}
...
async function runFolderList(
  ...
  const { body } = await webdavRequest(baseUrl, {
    ...headers,
    "Depth": "1",
  }, "PROPFIND", folderPath);
  const items = parsePropfindResponse(body);
  return items;
}
...
function parsePropfindResponse(xml: string): Record<string, unknown>[] {
  ...
  while ((m = getContentLength.exec(xml)) !== null) {
    lengths.push(m[1]);
  }
  const getContentType = /<d:getcontenttype[^>]*>([^<]*)<\/d:getcontenttype>/g;
  const contentTypes: string[] = [];
  while ((m = getContentType.exec(xml)) !== null) {
    contentTypes.push(m[1]);
  }
...
async function runUserInvite(
  ...
  if (groups && groups.length) params.set("groups", JSON.stringify(groups));
...
export const nextCloudExecutor: NodeExecutor = async (ctx, node) => {
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    try {
      ...
      const result = await runOperation(
        ctx, node, headers, baseUrl, resource, operation, itemJson, binaryData,
      );
