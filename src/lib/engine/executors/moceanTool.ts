...
export const moceanToolExecutor: NodeExecutor = async (ctx, node) => {
  ...
  if (!cred) {
    if (!continueOnFail) throw new Error("Mocean Tool: moceanApi credential is required");
    return [[{ json: { error: { message: "Mocean Tool: moceanApi credential is required" } } }]];
  }
  const apiKey = String((cred as Record<string, unknown>).apiKey ?? "");
  const apiSecret = String((cred as Record<string, unknown>).apiSecret ?? "");
  if (!apiKey || !apiSecret) {
    const msg = "Mocean Tool: apiKey and apiSecret are required in credential";
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    try {
      ...
      if (!from) throw new Error("Mocean Tool: 'from' parameter is required");
      if (!to) throw new Error("Mocean Tool: 'to' parameter is required");
      if (!message) throw new Error("Mocean Tool: 'message' parameter is required");
