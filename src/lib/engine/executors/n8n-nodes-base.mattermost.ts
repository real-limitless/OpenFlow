...
export const mattermostExecutor: NodeExecutor = async (ctx, node) => {
...
async function runChannelOperation(
  ...
  if (operation === "create") {
    const channelName = String(resolveValue(node.parameters.channelName, itemJson) ?? "");
    const displayName = String(resolveValue(node.parameters.displayName, itemJson) ?? "");
    const type = String(node.parameters.type ?? "O");
    if (!channelName) throw new Error("Mattermost: channelName is required");
    if (!displayName) throw new Error("Mattermost: displayName is required");
    const body: Record<string, unknown> = { name: channelName, display_name: displayName, type };
    ...
    const purpose = resolveValue(opts.purpose, itemJson);
    if (purpose) body.purpose = String(purpose);
    const header = resolveValue(opts.header, itemJson);
  ...
  if (operation === "search") {
    const channelName = String(resolveValue(node.parameters.channelName, itemJson) ?? "");
    if (!channelName) throw new Error("Mattermost: channelName is required");
    const body: Record<string, unknown> = { term: channelName };
    const res = await mattermostRequest(baseUrl, token, "POST", "/channels/search", body);
...
async function runMessageOperation(
  ...
  if (operation === "post") {
    ...
    const opts = (node.parameters.options ?? {}) as Record<string, unknown>;
    const props = resolveValue(opts.props, itemJson);
    if (props) {
    ...
    const fileIds = resolveValue(opts.fileIds, itemJson);
    if (fileIds) body.file_ids = String(fileIds).split(",").map((s: string) => s.trim());
...
async function runUserOperation(
  ...
  if (operation === "invite") {
    ...
    if (!userId) throw new Error("Mattermost: userId is required");
    const body: Record<string, unknown> = { team_id: teamId, user_id: userId };
