...
export const openWeatherMapToolExecutor: NodeExecutor = async (ctx) => {
  ...
  if (!apiKey) {
    throw new Error("OpenWeatherMapTool: API key is missing from credential");
  }
    ...
    try {
      ...
      if (locationType === "coordinates") {
        ...
        if (!cityName || cityName.trim() === "") {
          throw new Error("OpenWeatherMapTool: cityName is required when locationType is cityName");
        }
