...
export type NocoDbToolClient = {
  request(method: string, path: string, body?: unknown): Promise<{ status: number; body: unknown }>;
};

export type NocoDbToolClientFactory = (credentials: Record<string, unknown>) => Promise<NocoDbToolClient>;

let clientFactory: NocoDbToolClientFactory | null = null;

export function setNocoDbToolClientFactory(factory: NocoDbToolClientFactory | null): void {
  clientFactory = factory;
}

const DEFAULT_FACTORY: NocoDbToolClientFactory = async (credentials) => {
...
export const nocoDbToolExecutor: NodeExecutor = async (
